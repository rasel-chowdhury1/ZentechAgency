...
export default function ServicesPage() {
  ...
  useEffect(() => {
    // Hero: delay so browser paints opacity:0 before transitioning
    const t = setTimeout(() => heroRef.current?.classList.add("services-header-visible"), 120);

    // Grid: IntersectionObserver with threshold 0 (fires on first pixel visible)
    const el = gridRef.current;
    if (!el) return () => clearTimeout(t);
    const obs = new IntersectionObserver(
      ([e]) => { if (e.isIntersecting) { el.classList.add("cards-visible"); obs.disconnect(); } },
    ...
    <main className="min-h-screen bg-[color:var(--background)]">
      ...
      <div className="border-b border-[color:var(--border)]">
        <Container className="pt-6 pb-10 md:pt-8 md:pb-14">
          <div ref={heroRef}>
            ...
            >
              From strategy to deployment — we build, design, and scale your digital presence with precision and care.
            </p>

            {/* Stats row */}
            <div
              className="services-header-item mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-6"
      ...
      <Container className="py-12 md:py-16">
        <div ref={gridRef} className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {services.map((s, i) => (
            <Link key={s.slug} href={`/services/${s.slug}`} className="card-animated group">
              <div className="relative h-full overflow-hidden rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-6 transition-all duration-300 hover:-translate-y-1 hover:border-[color:var(--primary)]/40 hover:shadow-xl">
                ...
                <div className="relative z-10 flex items-start justify-between">
                  <div className="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-[color:var(--muted-bg)] ring-1 ring-[color:var(--border)] transition-all duration-300 group-hover:bg-[color:var(--primary)]/10 group-hover:ring-[color:var(--primary)]/30">
                    {icons[s.category]}
                  </div>
                  <span className="rounded-full bg-[color:var(--muted-bg)] px-2.5 py-0.5 text-[10px] font-medium text-[color:var(--muted)]">
                    {s.timeline}
                  </span>
                </div>
                ...
                {/* Title */}
                <h2 className="relative z-10 mt-1 text-[17px] font-semibold leading-snug">
                  {s.title}
                </h2>
                ...
                {/* Best for */}
                <div className="relative z-10 mt-3 flex items-center gap-1.5 text-[11px] text-[color:var(--muted)]">
                  <svg className="h-3.5 w-3.5 shrink-0 text-[color:var(--primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="relative z-10 mt-4 flex flex-wrap gap-1.5">
                  {s.bullets.slice(0, 3).map((b) => (
                    <span key={b} className="rounded-full bg-[color:var(--muted-bg)] px-2.5 py-0.5 text-[11px] font-medium text-[color:var(--muted)]">
                      {b}
                    </span>
                  ))}
                </div>
        ...
        <div className="mt-14 rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-8 text-center md:p-12">
          <p className="text-[11px] font-semibold uppercase tracking-widest text-[color:var(--primary)]">Get started</p>
          <h2 className="mt-2 text-2xl font-bold md:text-3xl">Not sure which service you need?</h2>
          <p className="mt-3 mx-auto max-w-md text-sm leading-relaxed text-[color:var(--muted)]">
            Book a free consultation and we'll help you find the right solution for your goals and budget.
          </p>
          <Link
            href="/contact"
